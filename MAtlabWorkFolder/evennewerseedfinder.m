%% Expanded RandomForest Experiment with Seed and Hyperparameter Sweep (with per-step reporting)
...
% MaxNumSplits) and prints the accuracy and sensitivity of every configuration.
% It stops as soon as it finds a configuration that meets both:
%   - Accuracy ≥ 70%
%   - Sensitivity ≥ 75%
%
% When a configuration meeting the criteria is found, the script prints and 
% records the random seed and hyperparameter settings.
%
% References:
...
useScreeningThreshold = false;      % Use a fixed screening threshold for evaluation
...
foundFlag = false;  % Flag to indicate if a desired configuration is found
finalSeed = NaN;
finalResult = [];
finalParams = struct();

expCount = 1;  % To count experiments
            ...
            for st = ScreeningThresholdOptions
                ...
                for np = NumPredictorsOptions
                    ...
                    for ms = MaxNumSplitsOptions
                        ...
                        % Print current results for this configuration
...
if foundFlag
    ...
    fprintf('\nNo configuration meeting both criteria was found after %d seeds and %d experiments.\n', maxSeeds, expCount);
end
...
function result = runRFExperiment(NTrees, MinLeafSize, screeningThreshold, useScreeningThreshold, infection_threshold, ...
    ...
    parfor i = 1:n_patients
        ...
        % Train the random forest model using the given hyperparameters.
