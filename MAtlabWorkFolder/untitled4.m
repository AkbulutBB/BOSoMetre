...
for i = 1:length(uniquePatients)
    ...
    % Reshape the predictions and scores to match the dimensions
    currentTrueLabels = testLabels{1}(:);  % Convert to column vector
    currentPredLabels = predLabels{1}(:);  % Convert to column vector
    currentScores = scores{1};  % Get the scores matrix
    
    % Concatenate results
    allTrueLabelsLSTM = [allTrueLabelsLSTM; currentTrueLabels];
    allPredictedLabelsLSTM = [allPredictedLabelsLSTM; currentPredLabels];
    
    % Handle scores concatenation
    if isempty(allScoresLSTM)
        ...
        if size(currentScores, 2) == size(allScoresLSTM, 2)
            ...
            error('Inconsistent number of classes in scores');
        end
    end
