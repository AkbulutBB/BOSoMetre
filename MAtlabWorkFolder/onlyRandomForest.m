%% RandomForest Experiment Using Old Parameter Settings
% This script performs a parameter sweep for a random forest (using TreeBagger)
% with LOOCV. Experimental parameters (e.g., NTrees, MinLeafSize, screeningThreshold)
% are partly inspired by your previous ensemble experiments.
%
% References:
...
fprintf('Starting Random Forest experiments at %s\n', datestr(now));
...
infection_threshold = 0.4;         % Infection threshold from the old analysis
useSMOTE = true;
k_neighbors = 5;
...
%% --- Parameter Grid ---
NTreesOptions = [50, 100, 200];           % Number of trees in the forest.
MinLeafSizeOptions = [1, 5, 10];            % Minimum observations per leaf.
screeningThresholdOptions = [0.3, 0.4, 0.5];% Fixed screening thresholds.
...
for nt = NTreesOptions
    for ml = MinLeafSizeOptions
        for st = screeningThresholdOptions
            fprintf('\nExperiment %d: NTrees = %d, MinLeafSize = %d, ScreeningThreshold = %.2f\n', ...
                    expCount, nt, ml, st);
...
writetable(resultsTable, 'rf_experiment_results.csv');
fprintf('\nAll experiment results exported to rf_experiment_results.csv\n');
diary off;
...
function result = runRFExperiment(NTrees, MinLeafSize, screeningThreshold, useScreeningThreshold, infection_threshold, ...
    features_all, binary_labels, patient_ids, batches, useDynamicWindow, minSize, maxSize, varThreshold, labelingMode)
    ...
    parfor i = 1:n_patients
        ...
        rfModel = trainRandomForestClassifier(X_train_scaled, y_train, NTrees, MinLeafSize);
...
function rfModel = trainRandomForestClassifier(X_train, y_train, NTrees, MinLeafSize)
    y_train_cat = categorical(y_train);
    opts = statset('UseParallel', true);
    rfModel = TreeBagger(NTrees, X_train, y_train_cat, 'Method', 'classification', ...
                'MinLeafSize', MinLeafSize, 'OOBPrediction', 'off', 'Options', opts);
end
...
function [preds, scores] = predictRandomForestClassifier(rfModel, X_test, infection_threshold)
    ...
    if size(scoresMat,2) == 1
        ...
        % Determine the column corresponding to positive class (assumed label '1').
